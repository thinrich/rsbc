module RsbcHelper
  ...
  # 1. The call is referencing an instance variable OR 
  ...
  def catch_nil(sexp, var)
    if sexp[1].nil?
      if @model.columns.map{|x| x.name.to_sym}.include? sexp[2] then   # columns = instance variables / this is the first case
        # 1.a if(!sexp[2].contains("_id"))
          pk("true")
        # 1.b else (AJAX)
          # For now just ignore all cases. THIS IS WRONG!!!
          # query database to see if sexp[2] exists and has contents? 
        return true
      else
        begin 
